...
window.init = function() {
  ...
  var isEnabledWebGL = !!canvas.getContext('webgl2'); // Browser Source on OBS doesn't support WebGL... (;_;)
        ...
        for (var i = 0; i < xml.length; i++) {
          ...
          if(bool1 || bool2) {
            ...
            if(isComment(xml[i].firstChild.nodeValue, SlashComment, BSPComment)) {
              var owner = xml[i].getAttributeNode('owner');
              if(OwnerComment==1 || !owner || owner.value!=1) {
                var handle='';
  ...
  function CanvasManager() {
    var self = this;
    this.faderTimer = -1;
    this.canUseWebGL = isEnabledWebGL;
